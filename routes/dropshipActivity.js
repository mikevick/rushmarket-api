...
const {
	getActivity,
	getTotals
} = require('../actions/dropshipActivity');
...
//
//  GET /dropshipActivity
//
router.get(`/`, jwtUtils.verifyToken, async (req, res, next) => {
	try {
		var dateFilterColumn = 'line_item_date_created';
		...
		if ((req.query.type === undefined) ||
			((req.query.type !== 'RETURNED') && (req.query.type !== 'SOLD') && (req.query.type !== 'TOTALS'))) {
			respond(resp, res, next, ["data"], 400, 'Type must be specified as RETURNED, SOLD or TOTALS.');
		}
			...
			if (req.query.vendorId) {
				if ((req.query.exactMatchFlag !== undefined) && (req.query.exactMatchFlag === 'true')) {
					whereInfo = sqlUtils.appendWhere(whereInfo, 'm.vendor_id LIKE ?', req.query.vendorId);
				} else {
					whereInfo = sqlUtils.appendWhere(whereInfo, 'm.vendor_id LIKE ?', req.query.vendorId + '%');
				}
			}
			...
			if (dateEnd.diff(dateStart, 'days') > 366) {
				...
				if (req.query.type !== 'TOTALS') {
					await getActivity(req.query.type, whereInfo, req.query.vendorId, req.query.source, resp);
				}
				else {
